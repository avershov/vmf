...
static void dumpStatistics( const MetadataStream& mdStream )
{
    std::cout << std::endl;
    ...
    for( int is = 0;  is < (int)schemaNames.size();  ++is )
    {
        ...
        std::shared_ptr< MetadataSchema > schema = mdStream.getSchema( schemaName );
        std::shared_ptr< Statistics > stat = schema->getStatistics();
        if( stat != nullptr )
        {
            std::cout << "statistics for schema '" << schemaName << "'" << std::endl;
            std::vector< StatisticsItem > items = stat->getStatisticsItems();
            if( items.size() != 0 )
            {
                for( int ii = 0;  ii < (int)items.size();  ++ii )
                {
                    const StatisticsItem& item = items[ii];
                    std::cout << "  name='" << item.name 
                              << "'  metadata='" << item.metadata 
                              << "'  field='" << item.field 
                              << "'  value=" << item.operation->getValue().get_integer() 
                              << std::endl;
                }
            }
            else
            {
                std::cout << "  no statistics items defined" << std::endl;
            }
        }
        else
        {
...
int main(int argc, char *argv[])
{
    ...
    const string GPS_STATS_COORD_NAME = "coord count";
    const string GPS_STATS_TIME_NAME = "time count";
    ...
    StatisticsItem si_coord( GPS_STATS_COORD_NAME, GPS_DESC, GPS_COORD_FIELD, IOperation::create( IOperation::CountId ));
    StatisticsItem si_time( GPS_STATS_TIME_NAME, GPS_DESC, GPS_TIME_FIELD, IOperation::create( IOperation::CountId ));
    shared_ptr<Statistics> stat( make_shared<Statistics>() );
    stat->addStatisticsItem( si_coord );
    stat->addStatisticsItem( si_time );
    mdStream.setStatistics( stat, GPS_SCHEMA_NAME );
